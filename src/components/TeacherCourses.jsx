...
function TeacherCourses({ baseURL }) {
  ...
  const fetchLessons = useCallback(async (courseId) => {
    ...
    const data = await apiRequest(`${baseURL}/courses/${courseId}`, "GET");
    if (data) {
  ...
  const fetchStudentProgress = useCallback(async (courseId) => {
    ...
    setError(null);
    setSelectedCourseId(courseId);
    const data = await apiRequest(`${baseURL}/progresses`, "GET");
    if (data) {
  ...
  return (
    <div>
      ...
      {selectedCourseId && studentProgress.length > 0 && (
        <div className="mt-6">
          <h3 className="text-lg font-semibold mb-2">Student Progresses for {studentProgress[0].enrollment.course.title}</h3>
          <ul className="space-y-2">
            {studentProgress.map((progress) => (
              ...
              <li key={progress._id} className="bg-white p-4 rounded shadow">
                ...
                <button id={progress.enrollment.student.public_id} onClick={handleViewMore}>
                View More
              </button>
              </li>
              </>
